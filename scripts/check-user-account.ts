...
async function checkUserAccount(emailOrName?: string) {
  ...
  if (!searchTerm) {
    console.log('Usage: npx tsx scripts/check-user-account.ts [email or name]')
    console.log('Example: npx tsx scripts/check-user-account.ts user@example.com')
    console.log('Example: npx tsx scripts/check-user-account.ts "John Doe"\n')
    return
  }
